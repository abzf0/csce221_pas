...
class BinarySearchTree
{
  ...
  private:
    ...
    node_ptr _root; // IMPORTANT
    size_type _size;
    key_compare comp;
    // why isn't this orange?

  public:
    BinarySearchTree() : _root(nullptr),_size(0),comp(Comparator()) {
        // TODO
    }
    BinarySearchTree( const BinarySearchTree & rhs ) : _root(clone(rhs._root)), _size(rhs._size),comp(rhs.comp) {
        // TODO
    }
  ...
  private:
    void destructorHelper(node_ptr & t) {
        if(t!=nullptr) {
            destructorHelper(t->left); destructorHelper(t->right);
            delete t;
            t = nullptr;
        }
    }
...
void printLevelByLevel( const BinarySearchTree<KK, VV, CC>& bst, std::ostream & out = std::cout ) {
    ...
    bool hasNonNullNextLevel = true;
    size_t nodesInCurrentLevel = 1;

    while (!q.empty() && hasNonNullNextLevel) {
        hasNonNullNextLevel = false;
        for (size_t i = 0; i < nodesInCurrentLevel; ++i) {
            ...
            if (current == nullptr) {
                ...
                printNode<KK, VV, CC>(out, *current) << " ";

                q.push(current->left);
                q.push(current->right);

                if (current->left != nullptr || current->right != nullptr)
                    hasNonNullNextLevel = true;
            }
        }
